...
// nestedFunction() can reach out and grab const internal from outside of its scope.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animalsArr){
    const displayNames = [];
    animalsArr.forEach(element => displayNames.push(`name: ${element.animal_name}, scientific: ${element.scientific_name}`))
    return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(animalsArr){
    const lowerCaseArr = animalsArr.map(element => element.animal_name.toLowerCase());
    return lowerCaseArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(animalsArr) {
    const filteredArr = animalsArr.filter(element => element.population < 5);
    return filteredArr;
  }
  ...
  */
  ...
  function USApop(animalsArr){
   const totalPop = animalsArr.reduce((acc, element) => {
